    ...
    <div className="bg-[#161b22] rounded-md border border-[#30363d] p-4">
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <img
          src={user.avatar_url}
          alt={user.login}
          className="w-20 h-20 rounded-full ring-1 ring-[#30363d] flex-shrink-0"
        />

        <div className="flex-1 min-w-0">
          {/* Name + username */}
          <h1 className="text-xl font-semibold text-[#e6edf3] truncate">
            {user.name || user.login}
          </h1>
          <p className="font-mono text-[#8b949e] text-sm">@{user.login}</p>
          ...
          <div className="flex flex-wrap gap-4 mt-3">
            {stats.map((s) => (
              <div key={s.label} className="flex items-center gap-1.5 text-sm">
                <span className="w-4 h-4 text-[#8b949e]">{s.icon}</span>
                <span className="font-mono font-semibold text-[#e6edf3]">{s.value.toLocaleString()}</span>
        ...
        <div className="flex-shrink-0 self-start">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-3 py-1 text-sm font-medium text-[#c9d1d9] bg-[#21262d] border border-[rgba(240,246,252,0.1)] rounded-md hover:bg-[#30363d] hover:border-[#8b949e] transition-colors duration-150"
          >
